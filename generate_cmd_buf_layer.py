...
def generate(file, output):
    ...
    for command in root.findall('commands/command'):
        ...
        if (name != None and ret_type != None and params != None and 'cmdbufferlevel' in command.attrib):
            ...
            print("    auto next_call = static_cast<PFN_{0}>(parent::get_next_device_proc_addr(\"{0}\"));".format(name.text), file=output)
...
def generate_cmd_buf(file, output):
    ...
    print("class cmd_buf_split_info : public T{", file=output)
    print("public:", file=output)
    print("using parent = T;", file=output)
